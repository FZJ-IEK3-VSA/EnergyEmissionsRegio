...
def plot_proxy_data(data_df, gdf, proxy_var_unit, save_path):
    merged_df = pd.merge(
...
def plot_solved_proxy_data(data_df, gdf, save_path):
    merged_df = pd.merge(
        gdf, data_df, left_on="code", right_on="region_code", how="left"
    )
    ...
    merged_df_de = merged_df[merged_df["code"].str.startswith("DE")]
    merged_df_de.plot(
        column="value", cmap="viridis_r", linewidth=0.8, ax=ax1, edgecolor="none"
    )
    ax1.axis("off")
    ...
    merged_df_es = merged_df[merged_df["code"].str.startswith("ES")]
    merged_df_es.plot(
        column="value", cmap="viridis_r", linewidth=0.8, ax=ax2, edgecolor="none"
    )
    ax2.axis("off")
...
def plot_validation_data(
    ...
    ax3.set_xlabel(f"Mean Absolute Error = {mae_de} {target_var_unit}", fontsize=15)
    ...
    ax4.set_xlabel(f"Mean Absolute Error = {mae_es} {target_var_unit}", fontsize=15)
